...
int staux_register_type(lua_State *L) {
    ...
    } /* else if (strcmp(type_value, STELLAR_TOBJECT) == 0) {
        if (lua_type(L, -3) != LUA_TTABLE) {
            luaL_error(L, "Expected an object (table) for field '%s'", field_name);
        }
        lua_pushnil(L);
        if (lua_next(L, -4) == 0) {
            luaL_error(L, "Object in field '%s' must not be empty", field_name);
        }
        lua_pop(L, 1);
        while (lua_next(L, -4)) {
            if (lua_type(L, -2) != LUA_TSTRING) {
                luaL_error(L, "Expected object (table with string keys) for field '%s'", field_name);
            }
            lua_pop(L, 1);
        }
    } */ else {
